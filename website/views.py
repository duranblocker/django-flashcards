...
def add(request):
	...
	if request.method == "POST":
		...
		old_num_2 = request.POST['old_num_2']
				...
				# Error Handling if no answer entered from form
		if not answer:
...
def divide(request):
	...
	if request.method == "POST":
		...
		if float(answer) == round(correct_answer, 2):
			my_answer = "Correct! " + str(round(correct_answer, 2)) + " = " + answer
			color = "success"
		else:
			my_answer = "Incorrect! " + old_num_1 + " / " + old_num_2 + " is " + str(round(correct_answer, 2)) + " not " + answer
			color = "danger"
